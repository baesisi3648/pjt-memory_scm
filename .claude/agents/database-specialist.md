...
- SQLModel ORM (async)
- Alembic (마이그레이션)
- aiosqlite (async SQLite driver, 프로덕션 시 asyncpg로 전환)
- 인덱스 최적화
- 커넥션 풀링 고려
...
- **교훈**: SQLModel 모델은 Base를 별도 파일로 분리하여 순환 참조 방지
```

SQLite → PostgreSQL 마이그레이션 고려사항:
- SQLite에서 지원하지 않는 기능 (JSONB, Array 등)은 PostgreSQL 전환 시 활용
- 개발 중에는 SQLite 호환 쿼리 사용, PostgreSQL 전용 기능은 주석으로 표기
- Full-text search: SQLite FTS5 → PostgreSQL tsvector/tsquery
- Connection pooling: aiosqlite → asyncpg pool

출력:
- SQLModel 모델 코드 (app/models/*.py)
